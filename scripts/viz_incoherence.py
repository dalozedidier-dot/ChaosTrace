...
# Code couleur explicite (tu peux changer si tu veux)
...
class Cfg:
    ...
    # Pour tenir en temps raisonnable : on évalue le gradient sur max_eval_points,
    # puis on interpole sur toute la timeline. Mets <=0 pour full résolution.
    max_eval_points: int = 300

    # seuils quantiles pour coder les niveaux
    q_stable: float = 0.50
    q_mild: float = 0.80
...
def build_multivariate_embedding(
    ...
    # Robust scaling (médiane + IQR) pour mixer des unités différentes
...
def plot_var(out_png: Path, t: np.ndarray, v: np.ndarray, inst: np.ndarray, lvl: np.ndarray, name: str, suffix: str):
    ...
    for k in range(4):
        m = lvl == k
        if np.any(m):
            ax1.scatter(t[m], v[m], s=8, alpha=0.85, color=LEVEL_COLORS[k], label=LEVEL_NAMES[k], zorder=3)
...
def main():
    ...
    pd.DataFrame(rows).sort_values("p95_contrib", ascending=False).to_csv(out_dir / "incoherence_summary.csv", index=False)
